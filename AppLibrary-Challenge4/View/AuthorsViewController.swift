...
class AuthorsViewController: UIViewController {
    ...
    lazy var autoresTableView : UITableView = UITableView()
    ...
    func initUI(){
        ...
        autoresTableView = UITableView(frame: CGRect(x: 0, y: 9*height/30, width: width, height: 18*height/30))
        autoresTableView.backgroundColor = .systemGray5
        autoresTableView.layer.cornerRadius = 20
        view.addSubview(autoresTableView)
        ...
        misLibrosLabel.text = Constants.ourAuthors
    ...
    @objc private func tabButtonPushed() {
        ...
        let vc : UIViewController = AddAuthorsViewController()
